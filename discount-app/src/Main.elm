...
init flags url navKey =
    let
        model =
            { amountType = AmountType.BeforeDiscount
            , amount = "100"
            , rate = "25"
            , navKey = navKey
            }
...
view model =
    { title = Texts.toString Texts.DiscountCalculatorT
    , body = [ content model ]
    }
        ...
        labelText =
            case model.amountType of
                AmountType.BeforeDiscount ->
                    Texts.AmountBeforeDiscountT

                AmountType.AfterDiscount ->
                    Texts.AmountAfterDiscountT
    in
    Input.text
        [ borderColor
        , Font.size 20
    ...
    let
        label =
            Input.labelAbove
                ...
                (Texts.toElement Texts.DiscountRateT)
    in
    Input.text
        [ borderColor
        , Font.size 20
    ...
    let
        ...
        optionAttributes amountType =
            let
                ...
                borderAttr =
                    if amountType == AmountType.BeforeDiscount then
                        Border.widthEach
                            { bottom = 1
                            , right = 1
                            , left = 1
        ...
        beforeDiscountOption =
            Element.el
                (optionAttributes AmountType.BeforeDiscount)
                (Element.el [ Element.centerX, Element.centerY ] (Texts.toElement Texts.BeforeDiscountT))

        afterDiscountOption =
            Element.el
                (optionAttributes AmountType.AfterDiscount)
                (Element.el [ Element.centerX, Element.centerY ] (Texts.toElement Texts.AfterDiscountT))
    in
    Element.column [ Element.width Element.fill, Element.spacing 10 ]
        [ Element.el [ Font.bold ] (Texts.toElement Texts.AmountTypeT)
        , Element.row [ Element.width Element.fill, Element.spacing 0 ] [ beforeDiscountOption, afterDiscountOption ]
        ]
